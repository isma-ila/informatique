...
def click_gauche(
        event):  # fonction rendant vivante la cellule cliquée donc met la valeur 1 pour la cellule cliquée au __dico_case
    x = event.x - (event.x % c)
    y = event.y - (event.y % c)
...
def click_droit(event):  # fonction tuant la cellule cliquée donc met la valeur 0 pour la cellule cliquée au __dico_case
    x = event.x - (event.x % c)
    y = event.y - (event.y % c)
